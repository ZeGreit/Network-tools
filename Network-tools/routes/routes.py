...
def main():
    ...
    user = 'username'
    passw = 'password'
    basedir = '/home/user/scripts/route'
    workdir = '%s/collect' % basedir
    out_dir = '%s/output' % basedir
    ...
    long_opt_list = ['all', 'IOS', 'nexus', 'CP', 'VSX', 'SRX', 'NS']
    try:
    ...
    get_routes(user, passw, basedir, workdir, opt_device, out_file, platform)
...
class router(object):
    ...
    def routing_table(misa, command):
        if misa.platform == 'nexus' or misa.platform == 'IOS':
            prompt = '%s#' % misa.hostname
            routes = get_info_cisco_juniper(misa.hostname, misa.user, misa.passw, command, prompt)
            return routes
    ...
    def vrf_list(misa, command):
        prompt = '%s#' % misa.hostname
        if misa.platform == 'nexus' or misa.platform == 'IOS':
            vrf_out = get_info_cisco_juniper(misa.hostname, misa.user, misa.passw, command, prompt)
...
class route(object):
    ...
    ##Format for IOS, nexus, SRX, NetScreen and Checkpoint standalone
    def format(misa, string):
        if misa.route_type == 'nexus' or misa.route_type == 'IOS':
...
def get_routes(user, passw, basedir, workdir, opt_device, out_file, platform):

    if platform:
        device_lists = ['list%s' % platform]
    ...
    for dev_list in device_lists:
        ...
        for hostname in devices:
            ...
            if platform == 'IOS' or platform == 'nexus':
            ...
            if platform == 'CP':
                route_command = 'chkp-search -cli %s "clish -c \\"show route\\""' % hostname
            ...
            if platform == 'CP-VSX':
                cp_user = 'username-of-mgmt-platform'
                cp_passw = 'password'
                cp_mgmt_srv = 'IP-of-mgmt-platform'
                ...
                route_command = 'vsx_util view_vs_conf -c %s -u %s -s 127.0.0.1--%s' % (hostname, cp_user, seq_nr)
...
#        dev_cli.sendline('set console page 0')
#        dev_cli.expect(prompt)
...
#-- Connect and get routes from IOS devices --#

def get_info_cisco_juniper(device, user, passw, command, prompt):
